...
if __name__ == "__main__":
    pages = sorted((ROOT / "pages_sur").glob("s*.png"))
    if len(sys.argv) > 1:
    ...
    for p in pages:
        ...
        rec["_usage"] = usage
        out.append(rec)
        (ROOT / "qwen-test" / f"raw_s{pg:02d}.txt").write_text(txt)
    (ROOT / "qwen-test" / "results_sur.json").write_text(json.dumps(out, indent=2))
    print(f"\nwrote qwen-test/results_sur.json ({len(out)} pages)")
